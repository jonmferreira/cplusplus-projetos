...
#include<vector>
#include<climits>
#include<queue>//tirar, usado pra testar logica
...
class StarTrek_PP1_Desfecho {
	...
	private:
		...
		void bfsBFS( vector<double>& distancias, vector<int>& visitado,
			...
			while( not filap.empty() and not fezexploracao ){
				
				Aresta 		aresta = filap.front();
		...
		void bfs(){
			...
			//queue é uma TAD pronta de fila, implemente o nosso!!!
			queue < Aresta > filap;
			filap.push( Aresta( posicaoAtual, distancias[ posicaoAtual ] ) );
			bfsBFS( distancias,visitados, filap );
		}
